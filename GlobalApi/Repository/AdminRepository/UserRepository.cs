    ...
    public class UserRepository: IUserRepository
    {
        ...
        public async Task<Profile> UpdateUserProfile(Profile_Image userProfile)
        {
            try
            {
                ...
                var user = await globalContext.Users.FirstOrDefaultAsync(y => y.Email == result.EmailID || y.PhoneNumber == result.Phonenumber);
        ...
        public async Task<Profile> GetUserByname(string username)
        {
            var user = await findUserId.FindUser(username);

            var profile = await globalContext.Profiles.FirstOrDefaultAsync(b => b.EmailID == (user.Email == null ? "" : user.Email) || b.Phonenumber == (user.PhoneNumber == null ? "" : user.PhoneNumber));
